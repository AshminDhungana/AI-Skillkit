...
Final report destination: `~/agents/output/report.md`
...
3.  **Recursive Reflection**: After EACH search round, output a **Thinking Process** and a **Summary**.
    ...
    - _Constraint_: Both sections must be **short and concise**.
...
- **Citation Format**: Use `[^index^]` for factual/formal pieces. Max **two citations per sentence**.
  - _Note_: Do not use citations for creative/non-formal writing.
- **Bolding Strategy**:
...
3. **Write**: Generate the article at `~/agents/output/report.md` following the word count and multi-paragraph subsection rules.
